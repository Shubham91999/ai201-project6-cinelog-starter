...
def test_add_to_watchlist_allows_private_entry(app, sample_user, sample_film):
    """Callers should be able to override the default visibility."""
    with app.app_context():
...
def test_get_watchlist_returns_newest_first(app, sample_user):
    """Watchlist entries should be returned newest-first by date_added."""
    with app.app_context():
...
def test_remove_from_watchlist_removes_entry(app, sample_user, sample_film):
    ...
    with app.app_context():
        ...
        assert remove_from_watchlist(user_id=sample_user, film_id=sample_film)
        assert WatchlistEntry.query.filter_by(
            user_id=sample_user, film_id=sample_film
        ).first() is None


def test_remove_from_watchlist_missing_entry_raises(app, sample_user):
